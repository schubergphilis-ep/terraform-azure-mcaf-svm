...
6. Run `terraform plan` and confirm no unexpected resource replacements — the underlying resources (`azapi_resource.subscription`, `restful_operation.subscription`, etc.) moved into submodules, so you will likely need `terraform state mv` (or `moved` blocks) to avoid a destroy/recreate. See the examples under `examples/{ea,mca,csp,existing}/` for full working configurations.
